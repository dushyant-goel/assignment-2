...
1.  \$\$
    ...
    (A^c)^c =\>\\ \omega \in \Omega : \omega \notin A\^c =\>\\ \omega \in A =\> \\ A

    \$\$

2.  $\phi$

3.  Suppose \$ x \in B\^c $\$ =\> x \notin B \$ --- 1\
    \$ A \subseteq B $\$ =\> \forall y \in B, y \in A \$\

Combining 1 and 2 \$ if y \notin B =\> y \notin A \$ --- 2\
\$ y \in B\^c =\> y \in A\^c $\$ B\^c \subseteq A\^c \$

4.  To Prove:\
    \$ (A \cap B)\^c = A\^c \cup B\^c \$\

Suppose,\
\$ x \in (A \cap B)\^c $\$ x \notin (A \cap B) $\$ x  is  not  in  A  and  x  is  not  in  B $\$ x \notin A  and  x \notin B \$\
\$ x \in A\^c and x \in B\^c \$\
\$ x \in A\^c \cap B\^c \$\

Expression for \$(\bigcup*{k=1}\^{K} A_k)\^c \$\
\$*\bigcap{k=1}\^{K} A_k \^c \$\
...
6.  Expression for \$(\bigcap*{k=1}\^{K} A_k)\^c \$\
    \$*\bigcup{k=1}\^{K} A_k \^c \$

## 2.2 (Q3)

\$ \|E\| := 2\^K - 1 \$ This is because every element in $\omega$ has $2$ options, - to be chosen as an element of a set, which is contained in E. - not be chosen.
...
3.  \$ 1\_{(A \cap B)\^c} =\> \$\
    ...
    \$ (1\_{A\^c}) + 1\_{B\^c} - 1\_{A^c}.1\_{B^c} \$ \$ 1\_{A\^c \cup B\^c} \$
...
We know, for sets \$ A \$ and \$ B $, \$ P(A \cup B) = P(A) + P(B) - P(A \cap B) \$   For disjoint sets $P(A \cap B) = 0$
